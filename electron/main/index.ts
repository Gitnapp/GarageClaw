...
import { ensureGarageClawContext, repairGarageClawOnlyBootstrapFiles } from '../utils/openclaw-workspace';
...
const WINDOWS_APP_USER_MODEL_ID = 'app.garageclaw.desktop';
...
if (process.platform === 'linux') {
  app.setDesktopName('garageclaw.desktop');
}
...
if (!gotElectronLock) {
  console.info('[GarageClaw] Another instance already holds the single-instance lock; exiting duplicate process');
  app.exit(0);
}
    ...
    if (!fileLock.acquired) {
      const ownerDescriptor = fileLock.ownerPid
        ...
        : fileLock.ownerFormat === 'unknown'
          ...
          : 'unknown owner';
      console.info(
        `[GarageClaw] Another instance already holds process lock (${fileLock.lockPath}, ${ownerDescriptor}); exiting duplicate process`,
      );
      app.exit(0);
    }
  } catch (error) {
    console.warn('[GarageClaw] Failed to acquire process instance file lock; continuing with Electron single-instance lock only', error);
  }
}
...
async function initialize(): Promise<void> {
  ...
  logger.init();
  logger.info('=== GarageClaw Application Starting ===');
  logger.debug(
  ...
  // Repair any bootstrap files that only contain GarageClaw markers (no OpenClaw
  // template content). This fixes a race condition where ensureGarageClawContext()
  // previously created the file before the gateway could seed the full template.
  void repairGarageClawOnlyBootstrapFiles().catch((error) => {
  ...
  gatewayManager.on('status', (status: { state: string }) => {
    ...
    if (status.state === 'running') {
      void ensureGarageClawContext().catch((error) => {
        logger.warn('Failed to re-merge GarageClaw context after gateway reconnect:', error);
      });
    }
  });
  ...
  // Merge GarageClaw context snippets into the workspace bootstrap files.
  // The gateway seeds workspace files asynchronously after its HTTP server
  // is ready, so ensureGarageClawContext will retry until the target files appear.
  void ensureGarageClawContext().catch((error) => {
    logger.warn('Failed to merge GarageClaw context into workspace:', error);
  });
