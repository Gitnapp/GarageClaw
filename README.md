...
<p align="center">
  <img src="src/assets/logo.svg" width="128" height="128" alt="GarageClaw Logo" />
</p>

<h1 align="center">GarageClaw</h1>
...
<p align="center">
  ...
  <a href="#why-garageclaw">Why GarageClaw</a> •
...
<p align="center">
  ...
  <img src="https://img.shields.io/github/downloads/ValueCell-ai/GarageClaw/total?color=%23027DEB" alt="Downloads" />
...
**GarageClaw** bridges the gap between powerful AI agents and everyday users. Built on top of [OpenClaw](https://github.com/OpenClaw), it transforms command-line AI orchestration into an accessible, beautiful desktop experience—no terminal required.

Whether you're automating workflows, managing AI-powered channels, or scheduling intelligent tasks, GarageClaw provides the interface you need to harness AI agents effectively.
...
Building AI agents shouldn't require mastering the command line. GarageClaw was designed with a simple philosophy: **powerful technology deserves an interface that respects your time.**

| Challenge | GarageClaw Solution |
|-----------|----------------|
...
GarageClaw is built directly upon the official **OpenClaw** core. Instead of requiring a separate installation, we embed the runtime within the application to provide a seamless "battery-included" experience.
...
When you target another agent with `@agent`, GarageClaw switches into that agent's own conversation context directly instead of relaying through the default agent. Agent workspaces stay separate by default, and stronger isolation depends on OpenClaw sandbox settings.
...
GarageClaw also pre-bundles full document-processing skills (`pdf`, `xlsx`, `docx`, `pptx`), deploys them automatically to the managed skills directory (default `~/.openclaw/skills`) on startup, and enables them by default on first install. Additional bundled skills (`find-skills`, `self-improving-agent`, `tavily-search`, `brave-web-search`) are also enabled by default; if required API keys are missing, OpenClaw will surface configuration errors in runtime.  
...
In **Settings → General**, you can enable **Launch at system startup** so GarageClaw starts automatically after login.
...
Download the latest release for your platform from the [Releases](https://github.com/ValueCell-ai/GarageClaw/releases) page.

#### Build from Source

```bash
# Clone the repository
git clone https://github.com/ValueCell-ai/GarageClaw.git
cd GarageClaw
...
When you launch GarageClaw for the first time, the **Setup Wizard** will guide you through:
...
> Note for Moonshot (Kimi): GarageClaw keeps Kimi web search enabled by default.  
> When Moonshot is configured, GarageClaw also syncs Kimi web search to the China endpoint (`https://api.moonshot.cn/v1`) in OpenClaw config.

### Proxy Settings

GarageClaw includes built-in proxy settings for environments where Electron, the OpenClaw Gateway, or channels such as Telegram need to reach the internet through a local proxy client.
...
- If advanced proxy fields are left empty, GarageClaw falls back to `Proxy Server`.
- Saving proxy settings reapplies Electron networking immediately and restarts the Gateway automatically.
- GarageClaw also syncs the proxy to OpenClaw's Telegram channel config when Telegram is enabled.
- Gateway restarts preserve an existing Telegram channel proxy if GarageClaw proxy is currently disabled.
...
GarageClaw employs a **dual-process architecture** with a unified host API layer. The renderer talks to a single client abstraction, while Electron Main owns protocol selection and process lifecycle:

```┌─────────────────────────────────────────────────────────────────┐
│                        GarageClaw Desktop App                         │
│                                                                  │
│  ┌────────────────────────────────────────────────────────────┐  │
...
- GarageClaw is an Electron app, so **one app instance normally appears as multiple OS processes** (main/renderer/zygote/utility). This is expected.
...
- Clicking the window close button (`X`) hides GarageClaw to tray; it does **not** fully quit the app. Use tray menu **Quit GarageClaw** for complete shutdown.
...
We welcome contributions from the community! Whether it's bug fixes, new features, documentation improvements, or translations—every contribution helps make GarageClaw better.
...
GarageClaw is built on the shoulders of excellent open-source projects:
...
### GarageClaw Partner Program 🚀

We're launching the GarageClaw Partner Program and looking for partners who can help introduce GarageClaw to more clients, especially those with custom AI agent or automation needs.

Partners help connect us with potential users and projects, while the GarageClaw team provides full technical support, customization, and integration.
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/GarageClaw&type=Date" alt="Star History Chart" />
</p>
...
GarageClaw is released under the [MIT License](LICENSE). You're free to use, modify, and distribute this software.
