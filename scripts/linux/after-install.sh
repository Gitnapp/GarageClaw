...
# Post-installation script for GarageClaw on Linux
...
# Create symbolic link for GarageClaw app binary
if [ -x /opt/GarageClaw/garageclaw ]; then
    ln -sf /opt/GarageClaw/garageclaw /usr/local/bin/garageclaw 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/GarageClaw/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/GarageClaw/chrome-sandbox' || true
else
    chmod 0755 '/opt/GarageClaw/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/GarageClaw/resources/apparmor-profile'
    APPARMOR_PROFILE_TARGET='/etc/apparmor.d/garageclaw'
...
echo "GarageClaw has been installed successfully."
