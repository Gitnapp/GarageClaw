; GarageClaw Custom NSIS Installer/Uninstaller Script
;
...
!macro customUnInstall
  ...
  MessageBox MB_YESNO|MB_ICONQUESTION \
    "Do you want to completely remove all GarageClaw user data?$\r$\n$\r$\nThis will delete:$\r$\n  • .openclaw folder (configuration & skills)$\r$\n  • AppData\Local\garageclaw (local app data)$\r$\n  • AppData\Roaming\garageclaw (roaming app data)$\r$\n$\r$\nSelect 'No' to keep your data for future reinstallation." \
    /SD IDNO IDYES _cu_removeData IDNO _cu_skipRemove

  _cu_removeData:
    ; Kill any lingering GarageClaw processes to release file locks on electron-store
    ; JSON files (settings.json, garageclaw-providers.json, window-state.json, etc.)
    ${nsProcess::FindProcess} "${APP_EXECUTABLE_FILENAME}" $R0
    ${if} $R0 == 0
      ${nsProcess::KillProcess} "${APP_EXECUTABLE_FILENAME}" $R0
    ${endIf}
    ...
    RMDir /r "$LOCALAPPDATA\garageclaw"
    RMDir /r "$APPDATA\garageclaw"
    ...
    ; Check AppData\Local\garageclaw
    IfFileExists "$LOCALAPPDATA\garageclaw\*.*" 0 _cu_localDone
      Sleep 3000
      RMDir /r "$LOCALAPPDATA\garageclaw"
      IfFileExists "$LOCALAPPDATA\garageclaw\*.*" 0 _cu_localDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$LOCALAPPDATA\garageclaw"'
        Pop $0
        Pop $1
    _cu_localDone:

    ; Check AppData\Roaming\garageclaw
    IfFileExists "$APPDATA\garageclaw\*.*" 0 _cu_roamingDone
      Sleep 3000
      RMDir /r "$APPDATA\garageclaw"
      IfFileExists "$APPDATA\garageclaw\*.*" 0 _cu_roamingDone
        nsExec::ExecToStack 'cmd.exe /c rd /s /q "$APPDATA\garageclaw"'
        Pop $0
        Pop $1
    _cu_roamingDone:
    ...
    IfFileExists "$PROFILE\.openclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $PROFILE\.openclaw"
    IfFileExists "$LOCALAPPDATA\garageclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $LOCALAPPDATA\garageclaw"
    IfFileExists "$APPDATA\garageclaw\*.*" 0 +2
      StrCpy $R3 "$R3$\r$\n  • $APPDATA\garageclaw"
  ...
  _cu_enumLoop:
    ...
    RMDir /r "$R2\.openclaw"
    RMDir /r "$R2\AppData\Local\garageclaw"
    RMDir /r "$R2\AppData\Roaming\garageclaw"
