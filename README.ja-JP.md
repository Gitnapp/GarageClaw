...
<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜgarageclawなのか">なぜGarageClawなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**GarageClaw**は、強力なAIエージェントと日常のユーザーとの間のギャップを埋めます。[OpenClaw](https://github.com/OpenClaw)をベースに構築されており、コマンドラインによるAIオーケストレーションを、アクセスしやすく美しいデスクトップ体験に変換します。ターミナルは不要です。

ワークフローの自動化、AI搭載チャネルの管理、インテリジェントなタスクのスケジューリングなど、GarageClawはAIエージェントを効果的に活用するために必要なインターフェースを提供します。

GarageClawはベストプラクティスのモデルプロバイダーが事前設定されており、Windowsおよび多言語設定をネイティブにサポートしています。もちろん、**設定 → 詳細設定 → 開発者モード**から高度な設定を微調整することもできます。

最近のリリースでは、既存のデスクトップ機能を置き換えずに Web プラットフォーム統合レイヤーも追加されました。ネイティブな OpenClaw ワークフローはそのままに、プラットフォームログイン、マーケットプレイス、credits、profile をデスクトップ内へ段階的に統合しています。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。GarageClawはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**

| 課題 | GarageClawのソリューション |
|------|----------------------|
...
GarageClawは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
`@agent` で別のエージェントを選ぶと、GarageClaw はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
...
GarageClaw はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。追加の同梱スキル（`find-skills`、`self-improving-agent`、`tavily-search`、`brave-web-search`）も既定で有効化されますが、必要な API キーが未設定の場合は OpenClaw が実行時に設定エラーを表示します。  
...
### 🌐 追加型 Web プラットフォーム統合
既存のネイティブデスクトップ体験を維持したまま、GarageClaw は次を追加しました。
- Web 上の Agents / Skills を閲覧するための Marketplace エントリ
- main-process host routes 経由でログイン状態と credits を表示するネイティブ Profile ページ
- renderer にプラットフォーム session を渡さずに使える、ログイン / profile / credits 用の埋め込み Web surface

### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。GarageClawはあなたの好みに自動的に適応します。

### 🚀 自動起動設定
**設定 → 通用** から **システム起動時に自動起動** を有効化すると、ログイン後に GarageClaw が自動的に起動します。
...
[Releases](https://github.com/ValueCell-ai/GarageClaw/releases)ページから、お使いのプラットフォーム向けの最新リリースをダウンロードしてください。
...
GarageClawを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：
...
GarageClawには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、GarageClawは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- GarageClawはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。
- GarageClawのプロキシが無効な状態では、Gatewayの通常再起動時に既存のTelegramチャネルプロキシ設定を保持します。
...
GarageClawは、**デュアルプロセス + Host API 統一アクセス**構成を採用しています。Renderer は単一クライアント抽象を呼び出し、プロトコル選択とライフサイクルは Main が管理します：

```┌─────────────────────────────────────────────────────────────────┐
│                        GarageClaw デスクトップアプリ                    │
│                                                                  │
│  ┌────────────────────────────────────────────────────────────┐  │
│  │              Electron メインプロセス                         │  │
...
- GarageClaw は Electron アプリのため、**1つのアプリインスタンスでも複数プロセス（main/renderer/zygote/utility）が表示される**のが正常です。
...
- ウィンドウの閉じるボタン（`X`）は既定でトレイへ最小化する動作で、完全終了ではありません。完全終了する場合はトレイメニューの **Quit GarageClaw** を使用してください。
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がGarageClawをより良くするのに役立ちます。
...
GarageClawは優れたオープンソースプロジェクトの上に構築されています：
...
### GarageClaw パートナープログラム 🚀

GarageClaw パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に GarageClaw を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、GarageClaw チームは技術サポート、カスタマイズ、統合を全面的に提供します。
...
<p align="center">
  <img src="https://api.star-history.com/svg?repos=ValueCell-ai/GarageClaw&type=Date" alt="スター履歴チャート" />
</p>
...
GarageClawは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
